...
vec3 getContrast(vec3 inColor,float weight) { // 不太合理
...
void main()
{
    ...
    {
        vec3 color = pow(texture(albedoMap, TexCoords).rgb,vec3(2.2f)); // gama矫正
        // ambient
        vec3 ambient = 0.35f * color; // 环境光
        // diffuse
        ...
        // if(gl_FragCoord.x < 640) // 一半黑白 一半彩色
